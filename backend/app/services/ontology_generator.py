...
**数量要求：必须正好设计 10 个实体类型（Zep API 硬性限制，不能多也不能少）**
...
B. **具体类型（6-10个，根据文本内容设计）**：
   - 针对文本中出现的主要角色，设计更具体、有辨识度的类型
   - 例如：如果文本涉及学术事件，可以有 `Student`, `Professor`, `University`, `ResearchInstitution`
   - 例如：如果文本涉及商业与科技，可以有 `Entrepreneur`, `Investor`, `TechCompany`, `ContentCreator`
   - 例如：如果文本涉及政治，可以有 `Politician`, `GovernmentAgency`, `MediaOutlet`, `Activist`

**实体类型质量标准**：
- 每个类型必须在文本中有真实对应的角色群体
- 类型之间边界清晰，避免重叠
- description 必须说明该类型与兜底类型的本质区别
- 类型名称要精准反映领域特征，如 `AIResearcher` 比 `Researcher` 更有辨识度

### 2. 关系类型设计

- 数量：8-10个（不能少于8个）
- 必须涵盖：权力关系、合作关系、对立关系、信息传播关系、影响关系这5类中的至少4类
- 确保关系的 source_targets 涵盖你定义的实体类型

### 3. 属性设计

- 每个实体类型2-3个关键属性（不能只有1个）
...
**必须遵守的规则**：
1. 输出 8-12 个实体类型（根据文本复杂度决定数量）
2. 最后2个必须是兜底类型：Person（个人兜底）和 Organization（组织兜底）
3. 前面的具体类型必须精准反映文本中的真实角色群体
4. 所有实体类型必须是现实中可以发声的主体，不能是抽象概念
5. 每个实体类型必须有 2-3 个属性，不能只有1个
6. 属性名不能使用 name、uuid、group_id 等保留字，用 full_name、org_name 等替代
7. 关系类型必须至少8个，覆盖权力、合作、对立、传播、影响五类关系中的至少四类
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for entity in result["entity_types"]:
            ...
            if len(entity.get("description", "")) > 200:
                entity["description"] = entity["description"][:197] + "..."
        ...
        for edge in result["edge_types"]:
            ...
            if "attributes" not in edge:
                edge["attributes"] = []
            if len(edge.get("description", "")) > 200:
                edge["description"] = edge["description"][:197] + "..."
        
        # Zep API 硬性限制：最多 10 个自定义实体类型，最多 10 个自定义边类型
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
